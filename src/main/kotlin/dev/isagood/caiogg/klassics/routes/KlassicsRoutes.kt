...
@Resource("/logout")
class Logout()
fun Route.klassics(dao: DAOFacade) {

    get<Logout> {
        call.sessions.clear<KlassicSession>()
        call.respondRedirect(application.href(Index()))
    }

    get<Index> {
        val session = call.sessions.get<KlassicSession>()
        val klassics = dao.getAll()
        val etag =
    ...
    get<KlassicPage> {
        val klassic = dao.klassicById(it.id)
    ...
    get<KlassicStream> {
        val klassic = dao.klassicById(it.id)
        println("Filename: ${klassic?.fileName}")
        if (klassic == null) {
